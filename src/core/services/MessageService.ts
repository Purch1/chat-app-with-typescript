...
export class MessageService {
  ...
  static async sendMessage({senderId, receiverId, content}: TCreateMessagePayload) {
    const conversation = await ConversationRepository.findByUser(senderId, receiverId);
    ...
    if (!conversation) {
      throw new Error('Failed to create or retrieve the conversation');
    }
    const newMessage = await MessageRepository.insert({ senderId, receiverId, content });

    if (newMessage) [conversation?.message.push(newMessage.id), await conversation.save()];
    return {
   ...
   */
  static async getByUser(userId: string): Promise<{ message: string; data: MessageResponseDto[] }> {
